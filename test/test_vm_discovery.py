...
from vagrantsandbox.vagrant_sandbox_provider import Vagrant, VagrantSandboxEnvironmentConfig
...
async def test_vm_discovery_single():
    ...
    with patch.object(vagrant, 'status', return_value=[{'name': 'default', 'state': 'not_created'}]):
...
async def test_vm_discovery_multi():
    ...
    with patch.object(vagrant, 'status', return_value=[
